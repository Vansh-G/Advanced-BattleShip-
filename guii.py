...
def draw_grid(player, left=0, top=0, search=False):
    for i in range(100):
        ...
        if search:
            ...
            y += SQ_SIZE // 2
            pygame.draw.circle(
                SCREEN, COLORS[player.search[i]], (x, y), radius=SQ_SIZE//4)
...
while animating:
    ...
    if not pausing:
        ...
        draw_grid(game.player2, search=True, left=(WIDTH-H_MARGIN) //
                  2 + H_MARGIN, top=(HEIGHT-V_MARGIN)//2+V_MARGIN)
